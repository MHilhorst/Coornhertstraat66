...
class Product { 
    ...
    this.get_product_info = async () => {
        const response = await fetch(`https://www.ah.nl/service/rest/delegate?url=producten/product/${this.product_id}`)
...
const all_products = ['wi210145/heineken-premium-pilsener',
...
const func = async (event, context) => {
    const response = await Promise.all(all_products.map(async item => {
        const new_product = new Product(item)
        await new_product.get_product_info()
        return {id:new_product.id,price:new_product.price,discount:new_product.discount,description:new_product.description,image:new_product.image}
    }))
